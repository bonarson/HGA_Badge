...
form.addEventListener("submit", function (e) {
  ...
  // Gestion du prénom avec initiale
  let affichagePrenom;
  if (prenom.includes(" ")) {
    const parties = prenom.split(" ");
    const initiale = parties[0].charAt(0).toUpperCase();
    const reste = parties.slice(1).join(" ");
    affichagePrenom = `${initiale}.${reste}`;
  } else {
  ...
  document.getElementById("Prenom").innerHTML = `📃 Prénom&nbsp;&nbsp;&nbsp;:&nbsp;&nbsp; ${affichagePrenom}`;
